...
describe 'Market Vendors API' do
  it 'can create market vendors' do
    ...
    post '/api/v0/market_vendors', headers:, params: JSON.generate(market_id: 322_474, vendor_id: 54_861)
    market_vendor = MarketVendor.last

    expect(response).to be_successful

    expect(market_vendor.market_id).to eq(322_474)
    expect(market_vendor.vendor_id).to eq(54_861)
  ...
  it 'displays error message if invalid id is passed' do
    ...
    post '/api/v0/market_vendors', headers:, params: JSON.generate(market_id: 123_498, vendor_id: 54_861)
